...
fit2015<- read_rds("data/generated_data/fit2015.rds")
...
#create a table for mean and SD
meanSD<- pdata %>%        mutate(GT=rgamma(1:nrow(.),shape=shape,scale=scale))  %>%
    group_by(year) %>% summarize(mean=mean(GT),sd=sd(GT))
