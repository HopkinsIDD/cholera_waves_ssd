...
### _Successive epidemic waves of cholera in South Sudan, 2014 - 2017_
...
path <- "data/lab_linelist/labsamplelinelist_2014_edited.xls"
...
library(lubridate)
path2017 <- "data/lab_linelist/Cholera lab specimen tracking sheet 25 Oct 2017_updated_edited.xlsx"
...
allLabll <- bind_rows(data2014_final,
                ...
                mutate(Total=Positive+Negative,
                       perPos=round(Positive/Total*100),
...
map <- read_sf("data/ssd_admbnda_adm2_200k_ssnbs_20160114.shp")
mapdf <- as.data.frame(map) %>% select(-geometry)
worldpop_dat <- read_csv("data/SSD15adjv3_tif+SSD-HDX-Borders_adm2_zonal.csv")

#bring in case data and break it up by week
master_dat <- read_rds("generated_data/master_linelist_simp.rds")
...
#bring in ocv data
ocv_dat <- read_xlsx("data/ocv_summary_south_sudan.xlsx",na = c("","?")) %>%
...
![](phylo_images\globaltree.cladeannot-01.png)
...
#load dataset
mlva_resistance <- read_csv("data/mlva_resistance_mlq.csv") %>%
...
mapdf <- as.data.frame(read_sf("data/ssd_admbnda_adm2_200k_ssnbs_20160114.shp")) %>%
        select(admin2Name,admin2Pcod)
chrps_rainfall_2010_2018 <- read_csv("data/ssd_chrps_county/chrps_counties_dailyrainfall_2010_2018.csv")
...
p2 <- master_dat %>% filter(!is.na(firstcase)) %>%
...
#see explore GT for original file
pdata <- read_csv("generated_data/pdata.csv")
discreteDF<- read_csv("generated_data/discreteDF.csv")
...
#bring in data
master_dat <- read_rds("generated_data/master_linelist_simp.rds")
nile <- read_rds("data/nile_river/nile.rds")
map <- read_sf("data/ssd_admbnda_adm2_200k_ssnbs_20160114.shp")
